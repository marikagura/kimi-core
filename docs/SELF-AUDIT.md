...
这个仓库本身就携带了构建它所用的*方法*：一套对抗式、multi-agent 的安全 + 去标识化（de-identification）审计，你可以在部署或公开之前，针对自己的 fork 运行一次。它是一组被指令做对抗式审计的 agent，**每一项发现在计入之前都要经过行为层面的验证。**

## 为什么要行为验证，而非静态推断

静态推断系统性地既产生误报又存在漏报。在本仓库自己的审计过程中，静态扫描产生了误报（一个“默认凭证”实际上是运行时经由 dotenv 加载的；一个 RLS 缺口其实早已封堵），同时漏掉了只有对运行中的产物执行 `grep` 才能抓到的残留。规则是：**任何发现在针对运行中的实体——一个请求、一次查询、一个文件——复现出来之前，都不予采信（或：都不成立）。**反驳（refutation）这一步才是整件事的要点；它能排除那些单次扫描会直接交付的“看似成立但实则错误”的发现。
...
> 对每一个 high/critical 级别的发现，派生一个独立的怀疑者（skeptic），它通过针对运行中的 service / DB / file 复现该发现来尝试**反驳**它。任何无法复现的都丢弃。只报告已确认的发现，外加每一项是如何被验证的。

然后修复、重新部署并再次运行。将“0 项确认”视为经验证所得的结果，而不是你假定的结果。
