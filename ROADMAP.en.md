...
- **SQLite / a zero-dependency "lite" mode — not doing it.** The three run paths today — local docker, self-hosted Postgres, managed Postgres (Supabase / Neon / … anything with `pgvector`) — already cover the single-user personal case. SQLite lite would only serve a narrow niche (won't touch docker, wants fully-local zero-server, and is semi-technical); the payoff is small, and the cost is a **second retrieval implementation** on SQLite (sqlite-vec + FTS5) mirroring the Postgres raw SQL (pgvector `<=>` / pg_trgm / pgroonga) — a permanent drift source that violates the repo's "single DRY scorer, no hand-copied drift" rule. Narrow benefit for a lasting maintenance burden — not worth it.
