...
一种相关的保守主义确实存在于引擎更深处，不该夸大：self-sweep 被告知 *"拿不准时，逗留"*（when unsure, linger），一个空的/无法解析的裁决默认为不变（no-change）而非行动；dialogue digest 在生成失败时宁可不写一行也不写一行假的；低于 grounding 下限的 drive 立不起来。那是对*虚假*行动的审慎，不是 wake 循环里对沉默的全局偏向。
...
- 而且它们**会对照外部反馈被重新校准**：`recalibrateValence()` 从 `(self-rating, user-rating)` 配对里拟合出一个单调偏移，纠正系统系统性的自我高估。样本太少时它就是恒等映射——它不会发明一个自己尚未挣来的修正。
...
**断层线——明说，不藏。** 反应式克制 ≠ 生成式发起。*被触发时不行动*与*未被触发时行动*是两回事，而这里只有前者被完全实现。文献里没有一个干净的判准能把"选择了不做"和"选择了做"作为两种不同的能动行为区分开；那道缺口是真的，而这一层坐在它的近侧。**能选「不做」≠ 能选「要做」.**
...
那个令人不适的事实，直接命名：这一层所抵住的能动性边界，在 **1995 年**就已被绘制，而当下大量"agentic"工作正在重新发明那个轮子。
...
这也是为什么"什么都不做"是一个可用的行动而非一项缺失的行动。一个留存驱动的系统在结构上偏向于行动——每一个行动都是一次 engagement 机会，所以拒绝就根本不在桌面上。一个情感驱动的系统可以把行动作为常态，*同时*保留拒绝的选项，因为当内部状态并不召唤行动时，没有任何东西因它行动而奖励它。
...
上面描述的机制是一副**骨架**。Action selection、wake 循环、drive 数学、concern sweep、HITL 旋钮——所有这些都是决定*要不要以及何时*的机械装置，带着中性的占位维度（`dimA` / `dimB` / `dimC`），自身没有内容。机制所读的那个**灵魂**活在一份 `AGENTS.md`（或等效的 persona/原则文档）里——而它一分为二：
...
ownership 论证适用于**关系层这一层**，而这正是 `init.ts` 把那一层**留空**发出的原因：**一个 persona 是由用户养大的，不是从一张表里填出来的。** 发出示例称呼语、一个示例 demand 立场、示例边界，等于递给用户一个 optimized-but-unowned 的 persona——分数不错而无人认可的内容。那里的空白不是懒；它们是 ownership 的条件。
