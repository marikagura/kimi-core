...
这是一个**个人单用户(1v1)的 agent memory OS** —— 不以多用户 / production 规模为目标。
...
- **更多投递 / 搜索后端预设。** 参考实现已发布(webhook notifier、http search provider)；更多开箱即用的具体后端（Slack / Discord / ntfy 预设、特定 search API 适配）欢迎，但不是必需。EXPLORE 的建议内容按设计留空（persona 层）。
- **发布跨版本的 eval 数字。** 把 `retrieval_eval` 的趋势随版本贴出来，仅用于展示，不影响引擎逻辑。
